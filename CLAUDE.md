...
```bash
uv run measuring-spoon build              # dist/ に STL を出力
uv run measuring-spoon build --help       # Param の各フィールドがそのままオプション
uv run measuring-spoon build out.stl      # 出力先は位置引数
...
- `src/measuring_spoon/main.py` — pydantic の `Param` と `build(param) -> cq.Workplane`。
  `build()` は純粋関数にする (ファイル I/O や表示を持ち込まない)。
- `src/measuring_spoon/__init__.py` — click の CLI。`@define_options(Param)` が
...
1. `uv run measuring_spoon build` を通す。
